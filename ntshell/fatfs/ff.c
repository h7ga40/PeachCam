...
#if FF_CODE_PAGE == 932	/* Japanese Shift-JIS */
...
#elif FF_CODE_PAGE == 855	/* Cyrillic */
#define _DF1S	0
...
#elif FF_CODE_PAGE == 860	/* Portuguese */
#define _DF1S	0
...
#elif FF_CODE_PAGE == 861	/* Icelandic */
#define _DF1S	0
...
#elif FF_CODE_PAGE == 863	/* Canadian-French */
#define _DF1S	0
...
#elif FF_CODE_PAGE == 1	/* ASCII (for only non-LFN cfg) */
#if FF_USE_LFN
#error Cannot use LFN feature without valid code page.
#endif
#define _DF1S	0

#elif FF_CODE_PAGE == 65001 /* UTF-8 */
#if FF_LFN_UNICODE
...
#if FF_VOLUMES < 1 || FF_VOLUMES > 9
#error Wrong FF_VOLUMES setting
#endif
static FATFS *FatFs[FF_VOLUMES];	/* Pointer to the file system objects (logical drives) */
static WORD Fsid;				/* File system mount ID */

#if FF_FS_RPATH && FF_VOLUMES >= 2
...
#if FF_USE_LFN == 0			/* Non LFN feature */
...
#endif
#if FF_USE_LFN == 1			/* LFN feature with static working buffer */
static WCHAR LfnBuf[FF_MAX_LFN + 1];
#define	DEFINE_NAMEBUF		BYTE sfn[12]
#define INIT_BUF(dobj)		{ (dobj).fn = sfn; (dobj).lfn = LfnBuf; }
#define	FREE_BUF()
#elif FF_USE_LFN == 2 		/* LFN feature with dynamic working buffer on the stack */
#define	DEFINE_NAMEBUF		BYTE sfn[12]; WCHAR lbuf[FF_MAX_LFN + 1]
#define INIT_BUF(dobj)		{ (dobj).fn = sfn; (dobj).lfn = lbuf; }
#define	FREE_BUF()
#elif FF_USE_LFN == 3 		/* LFN feature with dynamic working buffer on the heap */
#define	DEFINE_NAMEBUF		BYTE sfn[12]; WCHAR *lfn
#define INIT_BUF(dobj)		{ lfn = ff_memalloc((FF_MAX_LFN + 1) * 2); if (!lfn) LEAVE_FF((dobj).fs, FR_NOT_ENOUGH_CORE); (dobj).lfn = lfn; (dobj).fn = sfn; }
...
)
{
	...
	if (i == FF_FS_LOCK)	/* The object is not opened */
...
)
{
	...
	if (--i < FF_FS_LOCK) {	/* Shift index number origin from 0 */
		n = Files[i].ctr;
...
/*-----------------------------------------------------------------------*/
#if FF_FS_MINIMIZE <= 1 || FF_USE_LABEL || FF_FS_RPATH >= 2
static
FRESULT dir_read (
	FATFS_DIR* dp,		/* Pointer to the directory object */
...
)
{
	...
	while (dp->sect) {
		...
		a = dir[DIR_Attr] & AM_MASK;
#if FF_USE_LFN	/* LFN configuration */
		if (c == DDEM || (!FF_FS_RPATH && c == '.') || (int)((a & ~AM_ARC) == AM_VOL) != vol) {	/* An entry without valid data */
			ord = 0xFF;
		} else {
			if (a == AM_LFN) {			/* An LFN entry is found */
				if (c & LLEF) {			/* Is it start of LFN sequence? */
...
#else		/* Non LFN configuration */
		if (c != DDEM && (FF_FS_RPATH || c != '.') && a != AM_LFN && (int)((a & ~AM_ARC) == AM_VOL) == vol)	/* Is it a valid entry? */
			break;
#endif
		res = dir_next(dp, 0);				/* Next entry */
		if (res != FR_OK) break;
...
)
{
	...
	if (FF_FS_RPATH && (sn[NSFLAG] & NS_DOT))		/* Cannot create dot entry */
...
/*-----------------------------------------------------------------------*/
#if !FF_FS_READONLY && !FF_FS_MINIMIZE
static
		...
		for (;;) {
			...
			if (res != FR_OK) {				/* Failed to find the object */
				if (res == FR_NO_FILE) {	/* Object is not found */
					if (FF_FS_RPATH && (ns & NS_DOT)) {	/* If dot entry is not exist, */
...
)
{
	...
	static const char* const str[] = {FF_VOLUME_STRS};
	const char *sp;
	char c;
	TCHAR tc;
#endif
	...
	if (*path) {	/* If the pointer is not a null */
		for (tt = *path; (UINT)*tt >= (FF_USE_LFN ? ' ' : '!') && *tt != ':'; tt++) ;	/* Find ':' in the path */
	...
	*rfs = fs;							/* Return pointer to the file system object */
	...
	if (fs->fs_type) {					/* If the volume has been mounted */
		...
		if (!(stat & STA_NOINIT)) {		/* and the physical drive is kept initialized */
			if (!FF_FS_READONLY && wmode && (stat & STA_PROTECT))	/* Check write protection if needed */
	...
	if (!FF_FS_READONLY && wmode && (stat & STA_PROTECT))	/* Check disk write protection if needed */
		return FR_WRITE_PROTECTED;
#if FF_MAX_SS != FF_MIN_SS						/* Get sector size (multiple sector size cfg only) */
	if (disk_ioctl(fs->drv, GET_SECTOR_SIZE, &SS(fs)) != RES_OK
		|| SS(fs) < FF_MIN_SS || SS(fs) > FF_MAX_SS) return FR_DISK_ERR;
#endif
...
#if !FF_FS_READONLY
	mode &= FA_READ | FA_WRITE | FA_CREATE_ALWAYS | FA_OPEN_ALWAYS | FA_CREATE_NEW | FA_OPEN_APPEND;
...
)
{
	...
	if (res == FR_OK) {
		...
		if (FF_FS_RPATH && res == FR_OK && (djo.fn[NSFLAG] & NS_DOT))
			res = FR_INVALID_NAME;
#if FF_FS_LOCK != 0
...
)
{
...
#if FF_MAX_SS != FF_MIN_SS		/* Get disk sector size */
	if (disk_ioctl(pdrv, GET_SECTOR_SIZE, &SS(fs)) != RES_OK || SS(fs) > FF_MAX_SS || SS(fs) < FF_MIN_SS)
		return FR_DISK_ERR;
#endif
	if (FF_MULTI_PARTITION && part) {
	...
	if (au >= FF_MIN_SS) au /= SS(fs);	/* Number of sectors per cluster */
	if (!au) au = 1;
	if (au > 128) au = 128;
...
#if FF_USE_TRIM	/* Erase data area if needed */
	{

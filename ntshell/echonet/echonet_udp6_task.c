 ...
 *  の責任を負わない．
 * 
 *  @(#) $Id: echonet_udp6_task.c 1971 2019-07-10 04:36:59Z coas-nagasima $
 */

/*
 *		ECHONET Lite UDP通信処理タスク
 */
#if 0 //def SUPPORT_INET6
...
#define IPV6_ADDR_LOCAL_LOOPBACK_INIT	\
	{{{ UINT_C(0x00), UINT_C(0x00), UINT_C(0x00), UINT_C(0x00), \
	    UINT_C(0x00), UINT_C(0x00), UINT_C(0x00), UINT_C(0x00), \
	    UINT_C(0x00), UINT_C(0x00), UINT_C(0x00), UINT_C(0x00), \
	    UINT_C(0x00), UINT_C(0x00), UINT_C(0x00), UINT_C(0x01) }}}

static ECN_ENOD_ID udp_get_id(T_EDATA *edata, const T_IN6_ADDR ipaddr, uint16_t portno);
static int udp_get_ip(T_IPV6EP *fp_ipep, ECN_ENOD_ID fa_enodid);
 ...
 */
ER _ecn_udp2dtq(const uint8_t *buffer, size_t fa_len, const T_IPV6EP *dst);
ER _ecn_udp2dtq(const uint8_t *buffer, size_t fa_len, const T_IPV6EP *dst)
{
...
void echonet_udp_task(EXINF exinf)
{
	...
	get_tid(&tskid);
	memcpy(&a_src, in6_get_ifaddr(0), sizeof(T_IN6_ADDR));
	ECN_DBG_PUT_4("[UDP TSK:%d,%d] started, IP Address: %s ether_max_len: %u",
		tskid, exinf,
		ipv62str(NULL, &a_src),
		ETHER_MAX_LEN);
	memcpy(enodadrb_table[ENOD_LOCAL_ID].ipaddr, &a_src, sizeof(T_IN6_ADDR));
...
void _ecn_esv_msg(ECN_FBS_ID fbs_id)
{
	const static T_IN6_ADDR locl = IPV6_ADDR_LOCAL_LOOPBACK_INIT;
	/* UDP出力領域（暫定） */
	...
	memset(&a_dst.ipaddr, 0, sizeof(a_dst.ipaddr));
	a_dst.portno = 0;
	/* 送信先IPアドレス */
	...
	if (!a_ret) {
		...
		if (a_ret != E_OK) {
		...
		else if (memcmp(&a_dst.ipaddr, &locl, sizeof(a_dst.ipaddr)) == 0) {
			...
			ECN_DBG_PUT_2("[UDP TSK] udp6_snd_dat() to:%s %ubyte(s)",
				ipv62str(NULL, &a_dst.ipaddr), a_snd_len);
#ifdef ECN_DBG_PUT_ENA
			_ecn_dbg_bindmp((const uint8_t *)buffer, a_snd_len);
#endif
			/* UDP送信 */
			a_ret = udp6_snd_dat(ecn_udp_cepid, &a_dst, buffer, a_snd_len, TMO_FEVR);
			if (a_ret < 0) {
...
ER callback_nblk_udp(ID cepid, FN fncd, void *p_parblk)
{
	...
	case TFN_UDP_RCV_DAT:
		...
		if ((a_len = udp6_rcv_dat(cepid, &dst, buffer, sizeof(buffer) - 1, 20 * 1000)) < 0) {
		...
		ECN_CAP_PUT_4("[UDP ECHO SRV] recv, addr: %s:%d, len: %d, msg: '%s'",
			ipv62str(NULL, &dst.ipaddr), dst.portno, a_len, buffer);
...
{
	/*T_IN6_ADDR *ipaddr = (T_IN6_ADDR *)_ipaddr;*/
	ER			ret;
	T_ENUM_EPC	enm;
	uint8_t		epc;
	uint8_t		pdc;
 ...
 *  IPアドレスからリモートECHONETノードへ変換
 */
ECN_ENOD_ID udp_get_id(T_EDATA *edata, const T_IN6_ADDR ipaddr, uint16_t portno)
{
	T_ENOD_ADDR	*ea;
	const static T_IN6_ADDR mcst = IPV6_ADDR_LINKLOCAL_ALLNODES_INIT,
		locl = IPV6_ADDR_LOCAL_LOOPBACK_INIT;
	int			i;

	if (IN6_ARE_ADDR_EQUAL(&ipaddr, &locl))
		return ENOD_LOCAL_ID;
	if (IN6_ARE_ADDR_EQUAL(&ipaddr, in6_get_ifaddr(0)))
		return ENOD_LOCAL_ID;
	if (IN6_ARE_ADDR_EQUAL(&ipaddr, &mcst))
	...
	for (i = 0, ea = enodadrb_table; i < tnum_enodadr; i++, ea++) {
		if (!ea->inuse)
			continue;
		if (!IN6_ARE_ADDR_EQUAL(&ea->ipaddr, &ipaddr))
			continue;

		ECN_CAP_PUT_2("udp_get_id(): ip-found remote(%d) = %s",
			i - ENOD_REMOTE_ID, ipv62str(NULL, &ipaddr));
	...
	for (i = ENOD_REMOTE_ID, ea = &enodadrb_table[ENOD_REMOTE_ID]; i < tnum_enodadr; i++, ea++) {
		...
		if ((i - ENOD_REMOTE_ID + 1) >= tnum_enodid)
			break;
		if ((((T_IN6_ADDR *)ea->ipaddr)->__u6_addr.__u6_addr32[0] != 0)
			|| (((T_IN6_ADDR *)ea->ipaddr)->__u6_addr.__u6_addr32[1] != 0)
			|| (((T_IN6_ADDR *)ea->ipaddr)->__u6_addr.__u6_addr32[2] != 0)
			|| (((T_IN6_ADDR *)ea->ipaddr)->__u6_addr.__u6_addr32[3] != 0))
			continue;
		if (!ECHONET_NODE_MATCH(&eobjcb_table[i - ENOD_REMOTE_ID + 1], edata, &ipaddr, portno))
			continue;

		/* 対応するリモートノードがあればIPアドレスを設定 */
		memcpy(&ea->ipaddr, &ipaddr, sizeof(T_IN6_ADDR));
 ...
 */
int udp_get_ip(T_IPV6EP *fp_ipep, ECN_ENOD_ID fa_enodid)
{
	T_ENOD_ADDR *ea;
	const static T_IN6_ADDR mcst = IPV6_ADDR_LINKLOCAL_ALLNODES_INIT,
		locl = IPV6_ADDR_LOCAL_LOOPBACK_INIT;
	...
	if (fa_enodid == ENOD_MULTICAST_ID) {
		...
		memcpy(&fp_ipep->ipaddr, &mcst, sizeof(T_IN6_ADDR));
		return 0;	/* ok */
	}

	if (fa_enodid < ENOD_REMOTE_ID) {
		/* targetが未定義・LOCAL・APIの場合、ローカル配送を行う */
		memcpy(&fp_ipep->ipaddr, &locl, sizeof(T_IN6_ADDR));
		return 0;	/* ok */
	}
	...
	memcpy(&fp_ipep->ipaddr, &ea->ipaddr, sizeof(T_IN6_ADDR));
	return 0;	/* ok */
}
...
{
	return ipv62str(buf, (T_IN6_ADDR *)ipaddr);
}

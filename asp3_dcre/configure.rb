...
#  -n <net_proto>		ネットワーク層プロトコル（TINETが有効の場合は必須）
#						inet6、inet4、inet6/inet4、inet6m、inet6m/inet4 の何れかを指定する。
#  -s <trans_proto>		トランスポート層プロトコル（TINETが有効の場合は必須）
...
#  使用例(5)
#
#  % ../configure.rb -T gr_sakura_gcc -A sample1n \
#					-e tinet -i ether -v if_mbed -n inet4 -s tcp -U sample1.o
#	TINETを有効にする．
...
OptionParser.new(nil, 22) do |opt|
  ...
  opt.on("-i options",		"network interface (ether or ppp or loop)") do |val|
    $net_if = val
  end
  opt.on("-v options",		"ethernet device driver") do |val|
    $net_dev = val
  end
  opt.on("-n options",		"network layer protocol (inet6 or inet4 or inet6/inet4 or inet6m or inet6m/inet4)") do |val|
    $net_proto = val
  end
  opt.on("-s options",		"transeport layer protocol (tcp or udp or tcp/udp)") do |val|
    $trans_proto = val
  end
  opt.parse!(ARGV)
end
...
$vartable["SUPPORT_INET6"] = ($net_proto == "inet6") || ($net_proto == "inet6/inet4") || ($net_proto == "inet6m") || ($net_proto == "inet6m/inet4")
$vartable["SUPPORT_INET4"] = ($net_proto == "inet4") || ($net_proto == "inet6/inet4") || ($net_proto == "inet6m/inet4")
$vartable["SUPPORT_TCP"] = ($trans_proto == "tcp") || ($net_proto == "tcp/udp")

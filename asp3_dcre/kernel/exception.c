 ...
 *  の責任を負わない．
 * 
 *  $Id: exception.c 1484 2018-03-30 12:24:59Z coas-nagasima $
 */
...
initialize_exception(void)
{
	...
	for (i = 0; i < tnum_def_excno; i++) {
		p_excinib = &(excinib_table[i]);
		define_exc(p_excinib->excno, p_excinib->exc_entry);
	}
}
 ...
 *
 *  CPU例外ハンドラ中でenadspが変化することはないため，CPU例外が発生し
 *  た時のenadspを保存しておく必要はない．
 */

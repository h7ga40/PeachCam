 ...
 *  の責任を負わない．
 * 
 *  @(#) $Id: if_btusb.h 1484 2018-03-30 12:24:59Z coas-nagasima $
 */
...
#define IF_ETHER_NIC_GET_SOFTC()	btusb_get_softc()
#define IF_ETHER_NIC_WATCHDOG(i)	btusb_watchdog(i)
#define IF_ETHER_NIC_PROBE(i)		btusb_probe(i)
#define IF_ETHER_NIC_INIT(i)		btusb_init(i)
#define IF_ETHER_NIC_READ(i)		btusb_read(i)
#define IF_ETHER_NIC_RESET(i)		btusb_reset(i)
#define IF_ETHER_NIC_START(i,o)		btusb_start(i,o)

#define T_IF_ETHER_NIC_SOFTC		struct t_btusb_softc

/* IPv6 関係 */

#define IF_ETHER_NIC_IN6_IFID(i,a)	get_mac6_ifid(i,a)	/* インタフェース識別子の設定		*/
#define IF_ETHER_NIC_ADDMULTI(s)	btusb_addmulti(s)		/* マルチキャストアドレスの登録		*/
...
T_IF_SOFTC *btusb_get_softc(void);
...
void btusb_start(T_IF_SOFTC *ic, T_NET_BUF *output);

...
EOT
  ...
  def gen_through_cell_code( file )
    ...
    if @next_cell_port_subscript then
      subsc = "[#{@next_cell_port_subscript}]"
    else
      subsc = ""
    end
    file.print <<EOT
/*
 * generated by RPCPlugin
 *   start_region=#{@start_region.get_namespace_path}
 *   end_region  =#{@end_region.get_namespace_path}
 *   next_cell.entry=#{@next_cell.get_namespace_path}.#{@next_cell_port_name}#{subsc}
 */
...
#{indent_str}  #{@call_port_name} = #{@next_cell.get_namespace_path}.#{@next_cell_port_name}#{subscript};

 ...
 *  Implementation of the NetworkStack for the Socket
 */
class SocketInterface : public SocketStack, public NetworkInterface
{
public:
    /** SocketInterface lifetime
     */
    SocketInterface();
     ...
     *
     *  In Socket the callback will be called when processing OOB-messages via
    ...
    bool ntp(bool enabled, int timezone, const char *server0 = NULL, const char *server1 = NULL, const char *server2 = NULL);
    bool ntp_time(struct tm *tm);
    bool esp_time(struct timeval &tv);
    int ping(const char *addr);
    bool mdns(bool enabled, const char *hostname = NULL, const char *service = NULL, uint16_t portno = 0);
    bool mdns_query(const char *hostname, SocketAddress &addr);

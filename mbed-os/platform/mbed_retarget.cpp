...
namespace mbed{
MBED_WEAK FileHandle *mbed_target_override_console(int fd)
{
    return NULL;
}
...
extern "C" MBED_WEAK int socket_close(int fd)
{
    return -EINVAL;
}
...
extern "C" MBED_WEAK int socket_poll(struct pollfd *fds, nfds_t nfds, int timeout)
{
    errno = EINVAL;
    return -1;
}
...
{
    if (nfds > OPEN_MAX) {
        return socket_poll(fds, nfds, timeout);
    }

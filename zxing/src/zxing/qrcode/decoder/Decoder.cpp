...
int Decoder::correctErrors(ArrayRef<char> codewordBytes, int numDataCodewords)
{
	...
	int ret;
	if ((ret = rsDecoder_.decode(codewordInts, numECCodewords)) < 0)
		return ret;
...
int Decoder::decode(Ref<BitMatrix> bits, Ref<DecoderResult> &result)
{
	...
	Ref<FormatInformation> formatInformation;
	if ((ret = parser.readFormatInformation(formatInformation)) < 0)
		return ret;
	ErrorCorrectionLevel &ecLevel = formatInformation->getErrorCorrectionLevel();


	// Read codewords
	ArrayRef<char> codewords;
	...
	if ((ret = DataBlock::getDataBlocks(codewords, version, ecLevel, dataBlocks)) < 0)
		return ret;
	...
	for (size_t j = 0; j < dataBlocks.size(); j++) {
		...
		if ((ret = correctErrors(codewordBytes, numDataCodewords)) < 0)
			return ret;
		for (int i = 0; i < numDataCodewords; i++) {

...
BitMatrixParser::BitMatrixParser(Ref<BitMatrix> bitMatrix) :
	bitMatrix_(NULL), parsedVersion_(0), parsedFormatInfo_()
{
...
int BitMatrixParser::readFormatInformation(Ref<FormatInformation> &result)
{
	if (parsedFormatInfo_ != 0) {
		result = parsedFormatInfo_;
		return 0;
	}
...
{
	int ret;
	Ref<FormatInformation> formatInfo;
	if ((ret = readFormatInformation(formatInfo)) < 0)
		return ret;
	...
	bool readingUp = true;
	result = new Array<char>(version->getTotalCodewords());
	int resultOffset = 0;
	int currentByte = 0;
	int bitsRead = 0;

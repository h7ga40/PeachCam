...
int FinderPatternFinder::selectBestPatterns(vector<Ref<FinderPattern>> &result)
{
	...
	result = vector<Ref<FinderPattern>>(3);
...
int FinderPatternFinder::find(DecodeHints const &hints, Ref<FinderPatternInfo> &result)
{
	...
	int ret;
	vector<Ref<FinderPattern> > patternInfo;
	if ((ret = selectBestPatterns(patternInfo)) < 0)
		return ret;
	patternInfo = orderBestPatterns(patternInfo);

	result = new FinderPatternInfo(patternInfo);
	return 0;
}

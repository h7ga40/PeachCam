...
protected:
	...
	static int sampleGrid(Ref<BitMatrix> image, int dimension, Ref<PerspectiveTransform> transform, Ref<BitMatrix> &result);
	...
	int findAlignmentInRegion(float overallEstModuleSize, int estAlignmentX, int estAlignmentY,
		float allowanceFactor, Ref<AlignmentPattern> &result);
	int processFinderPatternInfo(Ref<FinderPatternInfo> info, Ref<DetectorResult> &result);
public:

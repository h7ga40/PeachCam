...
int Detector::detect(DecodeHints const &hints, Ref<DetectorResult> &result)
{
	...
	int ret;
	Ref<FinderPatternInfo> info;
	if ((ret = finder.find(hints, info)) < 0)
		return ret;
	return processFinderPatternInfo(info, result);
}

int Detector::processFinderPatternInfo(Ref<FinderPatternInfo> info, Ref<DetectorResult> &result)
{
	...
	if (provisionalVersion->getAlignmentPatternCenters().size() > 0) {
		...
		for (int i = 4; i <= 16; i <<= 1) {
			if ((ret = findAlignmentInRegion(moduleSize, estAlignmentX, estAlignmentY, (float)i, alignmentPattern)) < 0) {
				// try next round
			}
		}
	...
	int ret;
	Ref<BitMatrix> bits;
	if ((ret = sampleGrid(image_, dimension, transform, bits)) < 0)
		return ret;
	...
	result = new DetectorResult(bits, points);
	return 0;
}
...
int Detector::sampleGrid(Ref<BitMatrix> image, int dimension, Ref<PerspectiveTransform> transform, Ref<BitMatrix> &result)
{
	GridSampler &sampler = GridSampler::getInstance();
	return sampler.sampleGrid(image, dimension, transform, result);
}
...
int Detector::findAlignmentInRegion(float overallEstModuleSize, int estAlignmentX, int estAlignmentY,
	float allowanceFactor, Ref<AlignmentPattern> &result)
{
...
// should be
	...
	return alignmentFinder.find(result);
}

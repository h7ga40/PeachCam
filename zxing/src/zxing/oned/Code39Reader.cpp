...
int Code39Reader::decodeRow(int rowNumber, Ref<BitArray> row, Ref<Result> &rresult)
{
	...
	int ret;
	vector<int> start;
	if ((ret = findAsteriskPattern(row, theCounters, start)) < 0)
		return ret;
	...
	rresult = new Result(resultString, ArrayRef<char>(), resultPoints, BarcodeFormat::CODE_39);
	return 0;
}

int Code39Reader::findAsteriskPattern(Ref<BitArray> row, vector<int> &counters, vector<int> &result)
{
...
int Code39Reader::patternToChar(int pattern)
{
...
int Code39Reader::decodeExtended(std::string encoded, Ref<String> &decoded)
{

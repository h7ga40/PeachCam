...
int CodaBarReader::decodeRow(int rowNumber, Ref<BitArray> row, Ref<Result> &result)
{
	...
	if ((ret = validatePattern(startOffset)) < 0)
		return ret;
	...
	result = new Result(Ref<String>(new String(decodeRowResult)),
		ArrayRef<char>(),
		resultPoints,
...
int CodaBarReader::validatePattern(int start)
{
 ...
 */
int CodaBarReader::setCounters(Ref<BitArray> row)
{
	counterLength = 0;

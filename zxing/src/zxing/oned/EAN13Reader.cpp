	...
	std::string &resultString)
{
	...
	if ((ret = determineFirstDigit(resultString, lgPatternFound)) < 0)
		return ret;

	Range middleRange;
	if ((ret = findGuardPattern(row, rowOffset, true, MIDDLE_PATTERN, middleRange)) < 0)
		return ret;
...
int EAN13Reader::determineFirstDigit(std::string &resultString, int lgPatternFound)
{

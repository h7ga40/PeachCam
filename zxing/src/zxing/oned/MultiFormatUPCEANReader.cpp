...
int MultiFormatUPCEANReader::decodeRow(int rowNumber, Ref<BitArray> row, Ref<Result> &result)
{
// Compute this location once and reuse it on multiple implementations
	int ret;
	UPCEANReader::Range startGuardPattern;
	if ((ret = UPCEANReader::findStartGuardPattern(row, startGuardPattern)) < 0)
		return ret;
	for (int i = 0, e = readers.size(); i < e; i++) {
		Ref<UPCEANReader> reader = readers[i];
		int ret;
		if ((ret = reader->decodeRow(rowNumber, row, startGuardPattern, result)) < 0)
			continue;
		...
		if (ean13MayBeUPCA && canReturnUPCA) {
		  // Transfer the metdata across
			result = new Result(result->getText()->substring(1),
				result->getRawBytes(),

...
void GlobalHistogramBinarizer::initArrays(int luminanceSize)
{
	if (luminances->size() < luminanceSize) {
		luminances = new Array<char>(luminanceSize);
	}
...
int GlobalHistogramBinarizer::getBlackRow(int y, Ref<BitArray> row, Ref<BitArray> &result)
{
// std::cerr << "gbr " << y << std::endl;
	...
	initArrays(width);
	int ret;
	ArrayRef<char> localLuminances;
	if ((ret = source.getRow(y, luminances, localLuminances)) < 0)
		return ret;
	if (false) {
		std::cerr << "gbr " << y << " r ";
...
int GlobalHistogramBinarizer::getBlackMatrix(Ref<BitMatrix> &matrix)
{
	LuminanceSource &source = *getLuminanceSource();
	int width = source.getWidth();
	...
	for (int y = 1; y < 5; y++) {
		...
		int row = height * y / 5;
		ArrayRef<char> localLuminances;
		if ((ret = source.getRow(row, luminances, localLuminances)) < 0)
			return ret;
		int right = (width << 2) / 5;

...
int GenericMultipleBarcodeReader::decodeMultiple(Ref<BinaryBitmap> image,
	DecodeHints hints, vector<Ref<Result>> &results)
{
	int ret;
	if ((ret = doDecodeMultiple(image, hints, results, 0, 0, 0)) < 0)
		return ret;
	if (results.empty()) {
		return -1;
	}
...
int GenericMultipleBarcodeReader::doDecodeMultiple(Ref<BinaryBitmap> image,
	DecodeHints hints,
	...
	int currentDepth)
{
	...
	Ref<Result> result;
	int ret;
	if ((ret = delegate_.decode(image, hints, result)) < 0)
		return ret;
	...
	if (minX > MIN_DIMENSION_TO_RECUR) {
		if ((ret = doDecodeMultiple(image->crop(0, 0, (int)minX, height),
			hints, results, xOffset, yOffset, currentDepth + 1)) < 0)
			return ret;
	}
	// Decode above barcode
	if (minY > MIN_DIMENSION_TO_RECUR) {
		if ((ret = doDecodeMultiple(image->crop(0, 0, width, (int)minY),
			hints, results, xOffset, yOffset, currentDepth + 1)) < 0)
			return ret;
	}
	// Decode right of barcode
	if (maxX < width - MIN_DIMENSION_TO_RECUR) {
		if ((ret = doDecodeMultiple(image->crop((int)maxX, 0, width - (int)maxX, height),
			hints, results, xOffset + (int)maxX, yOffset, currentDepth + 1)) < 0)
			return ret;
	}
	// Decode below barcode
	if (maxY < height - MIN_DIMENSION_TO_RECUR) {
		if ((ret = doDecodeMultiple(image->crop(0, (int)maxY, width, height - (int)maxY),
			hints, results, xOffset, yOffset + (int)maxY, currentDepth + 1)) < 0)
			return ret;
	}
	return 0;
}

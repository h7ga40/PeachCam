...
private:
	int selectBestPatterns(std::vector<std::vector<Ref<zxing::qrcode::FinderPattern>>> &result);
...
public:
	...
	virtual int findMulti(DecodeHints const &hints, std::vector<Ref<zxing::qrcode::FinderPatternInfo>> &result);

...
int Decoder::decode(Ref<zxing::aztec::AztecDetectorResult> detectorResult, Ref<DecoderResult> &rresult)
{
	...
	Ref<BitArray> rawbits;
	if ((ret = extractBits(matrix, rawbits)) < 0)
		return ret;

	// std::printf("correcting bits\n");
	Ref<BitArray> aCorrectedBits;
	if ((ret = correctBits(rawbits, aCorrectedBits)) < 0)
		return ret;

	// std::printf("decoding bits\n");
	Ref<String> result;
	if ((ret = getEncodedData(aCorrectedBits, result)) < 0)
		return ret;
	...
	rresult = new DecoderResult(arrayOut, result);
	return 0;
}

int Decoder::getEncodedData(Ref<zxing::BitArray> correctedBits, Ref<String> &rresult)
{
	...
	rresult = new String(result);
	return 0;
}

int Decoder::correctBits(Ref<zxing::BitArray> rawbits, Ref<BitArray> &result)
{
//return rawbits;
// std::printf("decoding stuff:%d datablocks in %d layers\n", ddata_->getNBDatablocks(), ddata_->getNBLayers());
	...
	if ((ret = rsDecoder.decode(dataWords, numECCodewords)) < 0)
		return ret;
...
int Decoder::extractBits(Ref<zxing::BitMatrix> matrix, Ref<BitArray> &result)
{

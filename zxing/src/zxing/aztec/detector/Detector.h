...
private:
	...
	int extractParameters(std::vector<Ref<Point> > bullEyeCornerPoints);
	int getMatrixCornerPoints(std::vector<Ref<Point> > bullEyeCornerPoints, ArrayRef<Ref<ResultPoint>> &result);
	static int correctParameterData(Ref<BitArray> parameterData, bool compact);
	int getBullEyeCornerPoints(Ref<Point> pCenter, std::vector<Ref<Point>> &result);
	Ref<Point> getMatrixCenter();
	int sampleGrid(Ref<BitMatrix> image,
...
public:
	...
	int detect(Ref<AztecDetectorResult> &result);
};

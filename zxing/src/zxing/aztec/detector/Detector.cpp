...
int Detector::detect(Ref<AztecDetectorResult> &result)
{
	Ref<Point> pCenter = getMatrixCenter();
	int ret;
	std::vector<Ref<Point> > bullEyeCornerPoints;
	if ((ret = getBullEyeCornerPoints(pCenter, bullEyeCornerPoints)) < 0)
		return ret;

	extractParameters(bullEyeCornerPoints);

	ArrayRef< Ref<ResultPoint> > corners;
	if ((ret = getMatrixCornerPoints(bullEyeCornerPoints, corners)) < 0)
		return ret;

	Ref<BitMatrix> bits;
	if ((ret = sampleGrid(image_,
		corners[shift_ % 4],
		corners[(shift_ + 3) % 4],
		corners[(shift_ + 2) % 4],
		corners[(shift_ + 1) % 4],
	...
	result = new AztecDetectorResult(bits, corners, compact_, nbDataBlocks_, nbLayers_);
	return 0;
}

int Detector::extractParameters(std::vector<Ref<Point> > bullEyeCornerPoints)
{
...
int Detector::getMatrixCornerPoints(std::vector<Ref<Point> > bullEyeCornerPoints,
	ArrayRef<Ref<ResultPoint>> &array)
{
	...
	array = new Array< Ref<ResultPoint>>();
...
int Detector::correctParameterData(Ref<zxing::BitArray> parameterData, bool compact)
{
	int numCodewords;
	...
	if ((ret = rsDecoder.decode(parameterWords, numECCodewords)) < 0)
		return -1;
...
int Detector::getBullEyeCornerPoints(Ref<zxing::aztec::Point> pCenter,
...
Ref<Point> Detector::getMatrixCenter()
{
	...
	int ret;
	std::vector<Ref<ResultPoint> > cornerPoints;
	if ((ret = WhiteRectangleDetector(image_).detect(cornerPoints)) == 0) {
	...
	if ((ret = WhiteRectangleDetector(image_, 15, cx, cy).detect(cornerPoints)) == 0) {
		pointA = cornerPoints[0];
		pointB = cornerPoints[1];
		pointC = cornerPoints[2];
		pointD = cornerPoints[3];
...
int Detector::sampleGrid(Ref<zxing::BitMatrix> image,

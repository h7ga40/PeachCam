 ...
 */
int LinesSampler::sample(Ref<BitMatrix> &grid)
{
	...
	if ((ret = linesMatrixToCodewords(clusterNumbers, symbolsPerLine, symbolWidths, linesMatrix_, codewords)) < 0)
		return ret;
	...
	// XXX
	grid = new BitMatrix(dimension_, detectedCodeWords.size());
...
int LinesSampler::linesMatrixToCodewords(vector<vector<int> > &clusterNumbers,

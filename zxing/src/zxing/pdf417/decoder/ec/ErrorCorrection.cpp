...
int ErrorCorrection::decode(ArrayRef<int> received,
	int numECCodewords,
	ArrayRef<int> erasures)
{
	...
	if (error) {
		...
		int ret;
		Ref<ModulusPoly> modulusPoly;
		if ((ret = field_.buildMonomial(numECCodewords, 1, modulusPoly)) < 0)
			return ret;
		vector<Ref<ModulusPoly>> sigmaOmega;
		if ((ret = runEuclideanAlgorithm(modulusPoly, syndrome, numECCodewords, sigmaOmega)) < 0)
			return ret;
		...
		if ((ret = findErrorLocations(sigma, errorLocations)) < 0)
			return ret;
		ArrayRef<int> errorMagnitudes;
		if ((ret = findErrorMagnitudes(omega, sigma, errorLocations)) < 0)
			return ret;
...
int  ErrorCorrection::runEuclideanAlgorithm(Ref<ModulusPoly> a, Ref<ModulusPoly> b, int R, vector<Ref<ModulusPoly>> &v)
{
  // Assume a's degree is >= b's
	...
	while (r->getDegree() >= R / 2) {
		...
		while (r->getDegree() >= rLast->getDegree() && !r->isZero()) {
			...
			int ret;
			Ref<ModulusPoly> modulusPoly;
			if ((ret = field_.buildMonomial(degreeDiff, scale, modulusPoly)) < 0)
				return ret;
			q = q->add(modulusPoly);
	...
	v = vector<Ref<ModulusPoly>>(2);
	v[0] = sigma;
	v[1] = omega;
	return 0;
}

int ErrorCorrection::findErrorLocations(Ref<ModulusPoly> errorLocator, ArrayRef<int> &result)
{

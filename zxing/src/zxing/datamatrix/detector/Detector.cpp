...
int Detector::detect(Ref<DetectorResult> &result)
{
	...
	int ret;
	std::vector<Ref<ResultPoint> > ResultPoints;
	if ((ret = rectangleDetector_->detect(ResultPoints)) < 0)
		return ret;
	...
	if (4 * dimensionTop >= 7 * dimensionRight || 4 * dimensionRight >= 7 * dimensionTop) {
	  // The matrix is rectangular
		...
		transform = createTransform(topLeft, correctedTopRight, bottomLeft, bottomRight, dimensionTop,
			dimensionRight);
		if ((ret = sampleGrid(image_, dimensionTop, dimensionRight, transform, bits)) < 0)
			return ret;
   ...
   // The matrix is square
		...
		if ((ret = sampleGrid(image_, dimensionCorrected, dimensionCorrected, transform, bits)) < 0)
			return ret;
	}
...
int Detector::sampleGrid(Ref<BitMatrix> image, int dimensionX, int dimensionY,
	Ref<PerspectiveTransform> transform, Ref<BitMatrix> &result)
{
	GridSampler &sampler = GridSampler::getInstance();
	return sampler.sampleGrid(image, dimensionX, dimensionY, transform, result);
}

	...
	readBitMatrix_(NULL)
{
	...
	int ret;
	if ((ret = readVersion(bitMatrix, parsedVersion_)) < 0)
		return;

	if ((ret = extractDataRegion(bitMatrix, bitMatrix_)) < 0)
		return;

	readBitMatrix_ = new BitMatrix(bitMatrix_->getWidth(), bitMatrix_->getHeight());
}

int BitMatrixParser::readVersion(Ref<BitMatrix> bitMatrix, Ref<Version> &version)
{
	if (parsedVersion_ != 0) {
	...
	int ret = parsedVersion_->getVersionForDimensions(numRows, numColumns, version);
	if (ret != 0) {
		return 0;
	}
	return -1;
...
int  BitMatrixParser::readCodewords(ArrayRef<char> &result)
{
	result = new Array<char>(parsedVersion_->getTotalCodewords());
	int resultOffset = 0;
	int row = 4;
	int column = 0;
...
int BitMatrixParser::extractDataRegion(Ref<BitMatrix> bitMatrix, Ref<BitMatrix> &result)
{

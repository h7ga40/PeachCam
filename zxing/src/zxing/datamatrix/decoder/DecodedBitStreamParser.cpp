...
int DecodedBitStreamParser::decode(ArrayRef<char> bytes, Ref<DecoderResult> &dresult)
{
	...
	do {
		if (mode == ASCII_ENCODE) {
			...
			switch (mode) {
			case C40_ENCODE:
				if ((ret = decodeC40Segment(bits, result)) < 0)
					return ret;
				break;
			case TEXT_ENCODE:
				if ((ret = decodeTextSegment(bits, result)) < 0)
					return ret;
				break;
			case ANSIX12_ENCODE:
				if ((ret = decodeAnsiX12Segment(bits, result)) < 0)
					return ret;
				break;
			case EDIFACT_ENCODE:
				...
				break;
			case BASE256_ENCODE:
				if ((ret = decodeBase256Segment(bits, result, byteSegments)) < 0)
					return ret;
				break;
			default:
				return -1;
	...
	dresult = new DecoderResult(rawBytes, text);
	return 0;
}
...
int DecodedBitStreamParser::decodeC40Segment(Ref<BitSource> bits, ostringstream &result)
{
...
int DecodedBitStreamParser::decodeTextSegment(Ref<BitSource> bits, ostringstream &result)
{
...
int DecodedBitStreamParser::decodeAnsiX12Segment(Ref<BitSource> bits, ostringstream &result)
{
...
int DecodedBitStreamParser::decodeBase256Segment(Ref<BitSource> bits, ostringstream &result, vector<char> byteSegments)
{

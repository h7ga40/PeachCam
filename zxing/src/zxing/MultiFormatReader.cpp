...
int MultiFormatReader::decode(Ref<BinaryBitmap> image, Ref<Result> &result)
{
	setHints(DecodeHints::DEFAULT_HINT);
	return decodeInternal(image, result);
}

int MultiFormatReader::decode(Ref<BinaryBitmap> image, DecodeHints hints, Ref<Result> &result)
{
	setHints(hints);
	return decodeInternal(image, result);
}

int MultiFormatReader::decodeWithState(Ref<BinaryBitmap> image, Ref<Result> &result)
{
...
int MultiFormatReader::decodeInternal(Ref<BinaryBitmap> image, Ref<Result> &result)
{
	for (unsigned int i = 0; i < readers_.size(); i++) {
		if (readers_[i]->decode(image, hints_, result) == 0)
			return 0;
	}
	return -1;
}

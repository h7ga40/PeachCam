...
int8_t ESP32::getRSSI()
{
    ...
    startup();
    ret = _parser.send("AT+CWJAP?")
       && _parser.recv("+CWJAP:\"%32[^\"]\",\"%17[^\"]\",%hhd,%hhd\r\nOK", ssid, bssid, &channel, &rssi);
    _smutex.unlock();
...
bool ESP32::socket_setopt(int id, const char *optname, int optval)
{
    bool ret;
    ...
    _smutex.lock();
    ret = _parser.send("AT+CIPSETOPT=%d,\"%s\",%d", id, optname, optval)
        && _parser.recv("OK");
    _smutex.unlock();
...
bool ESP32::socket_setopt(int id, const char *optname, const char *optval)
{
    bool ret;
    ...
    _smutex.lock();
    ret = _parser.send("AT+CIPSETOPT=%d,\"%s\",%s", id, optname, optval)
        && _parser.recv("OK");
    _smutex.unlock();
...
bool ESP32::socket_getopt(int id, const char *optname, int *optval)
{
    bool ret;
    ...
    _smutex.lock();
    ret = _parser.send("AT+CIPGETOPT=%d,\"%s\"", id, optname)
        && _parser.recv("+CIPGETOPT:%d\r\nOK", optval);
    _smutex.unlock();

    return ret;
}

bool ESP32::ntp(bool enabled, int timezone, const char *server0 , const char *server1, const char *server2)
{
    bool ret;
    ...
    if (server0 != NULL) {
        if (server1 != NULL) {
            if (server2 != NULL) {
                ret = _parser.send("AT+CIPSNTPCFG=%d,%d,\"%s\",\"%s\",\"%s\"", enabled ? 1 : 0, timezone, server0, server1, server2)
                    && _parser.recv("OK");
            }
            else {
                ret = _parser.send("AT+CIPSNTPCFG=%d,%d,\"%s\",\"%s\"", enabled ? 1 : 0, timezone, server0, server1)
                    && _parser.recv("OK");
            }
        }
        else {
            ret = _parser.send("AT+CIPSNTPCFG=%d,%d,\"%s\"", enabled ? 1 : 0, timezone, server0)
                && _parser.recv("OK");
        }
    }
    else {
        ret = _parser.send("AT+CIPSNTPCFG=%d,%d", enabled ? 1 : 0, timezone)
            && _parser.recv("OK");
    }
    _smutex.unlock();
...
bool ESP32::ntp_time(struct tm *tm)
{
    bool ret;
    char mon[4], week[4];
    ...
    ret = _parser.send("AT+CIPSNTPTIME?")
       && _parser.recv("+CIPSNTPTIME:%4s %4s %d %d:%d:%d %d\r\nOK", week, mon, &tm->tm_mday, &tm->tm_hour, &tm->tm_min, &tm->tm_sec, &tm->tm_year);
    _smutex.unlock();
    ...
    tm->tm_year -= 1900;
    switch (CHAR3_TO_INT(mon[0], mon[1], mon[2])) {
    case CHAR3_TO_INT('J', 'a', 'n'): tm->tm_mon = 0; break;
    case CHAR3_TO_INT('F', 'e', 'b'): tm->tm_mon = 1; break;
    case CHAR3_TO_INT('M', 'a', 'r'): tm->tm_mon = 2; break;
    case CHAR3_TO_INT('A', 'p', 'r'): tm->tm_mon = 3; break;
    case CHAR3_TO_INT('M', 'a', 'y'): tm->tm_mon = 4; break;
    case CHAR3_TO_INT('J', 'u', 'n'): tm->tm_mon = 5; break;
    case CHAR3_TO_INT('J', 'u', 'l'): tm->tm_mon = 6; break;
    case CHAR3_TO_INT('A', 'u', 'g'): tm->tm_mon = 7; break;
    case CHAR3_TO_INT('S', 'e', 'p'): tm->tm_mon = 8; break;
    case CHAR3_TO_INT('O', 'c', 't'): tm->tm_mon = 9; break;
    case CHAR3_TO_INT('N', 'o', 'v'): tm->tm_mon = 10; break;
    case CHAR3_TO_INT('D', 'e', 'c'): tm->tm_mon = 11; break;
    }

    return ret;
}

bool ESP32::esp_time(struct timeval &tv)
{
    bool ret;

    _smutex.lock();
    ret = _parser.send("AT+SYSTIME?")
        && _parser.recv("+SYSTIME:%ld.%lu\r\nOK", &tv.tv_sec, &tv.tv_usec);
    _smutex.unlock();

    return ret;
}

int ESP32::ping(const char *addr)
{
    int ret;
    ...
    _smutex.lock();
    ret = _parser.send("AT+PING=\"%s\"", addr)
        && _parser.recv("+PING:%d\r\nOK", &ret);
    _smutex.unlock();
...
bool ESP32::mdns(bool enabled, const char *hostname, const char *service, uint16_t portno)
{
    int ret;
    ...
    if (enabled) {
        if (hostname == NULL) {
            ...
            ret = _parser.send("AT+MDNS=1,\"%s\",\"%s\",%d", hostname, service, portno)
               && _parser.recv("OK");
        }
    }
    else {
...
bool ESP32::mdns_query(const char *hostname, SocketAddress &addr)
{
    int ret;
    char temp[32];
    ...
    _smutex.lock();
    ret = _parser.send("AT+CIPQRYHST=\"%s\"", hostname)
        && _parser.recv("+CIPQRYHST:\"%32[^\"]\"", temp)
        && _parser.recv("OK");
    _smutex.unlock();

    if (ret)
        addr.set_ip_address(temp);

    return ret;
}

bool ESP32::sleep(bool enebled)
{
    int ret;

    _smutex.lock();
    ret = _parser.send("AT+SLEEP=%d", enebled ? 1 : 0)
        && _parser.recv("OK", &ret);
    _smutex.unlock();

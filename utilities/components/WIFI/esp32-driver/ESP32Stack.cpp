    ...
    int optname, const void *optval, unsigned optlen)
{
    ...
    switch (optname) {
            ...
            if (level == NSAPI_SOCKET && socket->proto == NSAPI_TCP) {
                ...
                if (optlen == sizeof(int)) {
                    ...
                    if (secs >= 0 && secs <= 7200) {
                        ...
                        _esp->socket_setopt(socket->id, "TCP_KEEPALIVE", secs);
        ...
        case NSAPI_REUSEADDR:
            if (level == NSAPI_SOCKET && socket->proto == NSAPI_TCP) {
                int val = *(int *)optval;
                if (_esp->socket_setopt(socket->id, "SO_REUSEADDR", val)) {
                    return NSAPI_ERROR_OK;
                }
            }
            break;
        case NSAPI_KEEPIDLE:
            if (level == NSAPI_SOCKET && socket->proto == NSAPI_TCP) {
                int val = *(int *)optval;
                if (_esp->socket_setopt(socket->id, "TCP_KEEPIDLE", val)) {
                    return NSAPI_ERROR_OK;
                }
            }
            break;
        case NSAPI_KEEPINTVL:
            if (level == NSAPI_SOCKET && socket->proto == NSAPI_TCP) {
                int val = *(int *)optval;
                if (_esp->socket_setopt(socket->id, "TCP_KEEPINTVL", val)) {
                    return NSAPI_ERROR_OK;
                }
            }
            break;
        ...
        case NSAPI_ADD_MEMBERSHIP:
            if (level == NSAPI_SOCKET && socket->proto == NSAPI_UDP) {
                ...
                if (_esp->socket_setopt(socket->id, "IP_ADD_MEMBERSHIP", temp)) {
                    return NSAPI_ERROR_OK;
                }
            }
            break;
        case NSAPI_DROP_MEMBERSHIP:
            if (level == NSAPI_SOCKET && socket->proto == NSAPI_UDP) {
                ...
                SocketAddress imr_multiaddr(mreq->imr_multiaddr);
                snprintf(temp, sizeof(temp) - 1, "\"%s\",\"%s\"", imr_interface.get_ip_address(), imr_multiaddr.get_ip_address());
                if (_esp->socket_setopt(socket->id, "IP_DROP_MEMBERSHIP", temp)) {
                    return NSAPI_ERROR_OK;
                }
            }
            break;
        case NSAPI_NODELAY:
            if (level == NSAPI_SOCKET && socket->proto == NSAPI_TCP) {
                int val = *(int *)optval;
                if (_esp->socket_setopt(socket->id, "TCP_NODELAY", val)) {
                    return NSAPI_ERROR_OK;
                }
            }
            break;
        case NSAPI_SO_KEEPALIVE:
            if (level == NSAPI_SOCKET && socket->proto == NSAPI_TCP) {
                int val = *(int *)optval;
                if (_esp->socket_setopt(socket->id, "SO_KEEPALIVE", val)) {
                    return NSAPI_ERROR_OK;
                }
            }
            break;
        case NSAPI_SO_RCVTIMEO:
            if (level == NSAPI_SOCKET && socket->proto == NSAPI_TCP) {
                int val = *(int *)optval;
                if (_esp->socket_setopt(socket->id, "SO_RCVTIMEO", val)) {
                    return NSAPI_ERROR_OK;
                }
            }
            break;
    ...
    int optname, void *optval, unsigned *optlen)
{
    ...
    switch (optname) {
        ...
        case NSAPI_REUSEADDR:
            if (level == NSAPI_SOCKET && socket->proto == NSAPI_TCP) {
                int val = *(int *)optval;
                if (_esp->socket_getopt(socket->id, "SO_REUSEADDR", &val)) {
                    return NSAPI_ERROR_OK;
                }
            }
            break;
        case NSAPI_KEEPIDLE:
            if (level == NSAPI_SOCKET && socket->proto == NSAPI_TCP) {
                int val = *(int *)optval;
                if (_esp->socket_getopt(socket->id, "TCP_KEEPIDLE", &val)) {
                    return NSAPI_ERROR_OK;
                }
            }
            break;
        case NSAPI_KEEPINTVL:
            if (level == NSAPI_SOCKET && socket->proto == NSAPI_TCP) {
                int val = *(int *)optval;
                if (_esp->socket_getopt(socket->id, "TCP_KEEPINTVL", &val)) {
                    return NSAPI_ERROR_OK;
                }
            }
            break;
        ...
        case NSAPI_NODELAY:
            if (level == NSAPI_SOCKET && socket->proto == NSAPI_TCP) {
                int val = *(int *)optval;
                if (_esp->socket_getopt(socket->id, "TCP_NODELAY", &val)) {
                    return NSAPI_ERROR_OK;
                }
            }
            break;
        case NSAPI_ERROR:
            if (level == NSAPI_SOCKET && socket->proto == NSAPI_TCP) {
                int val = *(int *)optval;
                if (_esp->socket_getopt(socket->id, "SO_ERROR", &val)) {
                    return NSAPI_ERROR_OK;
                }
            }
            break;
        case NSAPI_SO_KEEPALIVE:
            if (level == NSAPI_SOCKET && socket->proto == NSAPI_TCP) {
                int val = *(int *)optval;
                if (_esp->socket_getopt(socket->id, "SO_KEEPALIVE", &val)) {
                    return NSAPI_ERROR_OK;
                }
            }
            break;
        case NSAPI_SO_RCVTIMEO:
            if (level == NSAPI_SOCKET && socket->proto == NSAPI_TCP) {
                int val = *(int *)optval;
                if (_esp->socket_getopt(socket->id, "SO_RCVTIMEO", &val)) {
                    return NSAPI_ERROR_OK;
                }
            }
            break;
